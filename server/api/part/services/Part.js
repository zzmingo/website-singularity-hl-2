...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Part._attributes)
      .filter(attribute => attribute !== Part.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Part._attributes[attribute].type));
